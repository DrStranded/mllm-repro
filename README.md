...
Reproduction package for the vision-language experiments: GT-Reward, TTRL, and
co-learning on open-r1 and MMR1, at the small tier (2B and 4B backbones) and the
big tier (7B to 12B). Every run in the paper maps to one launcher under
`examples/`; the environment is pinned by a full dependency freeze and replayed
by the Dockerfile.

This README is a runbook. Steps are explicit commands, paths are repo-relative,
and stack versions and hyperparameters are frozen (section 4); do not substitute
newer versions. Where a step says "override via env", export the variable before
calling the launcher so it takes precedence over the built-in default.
...
Do steps AH. A/B are one-time setup; F/G/H repeat per experiment.
...
Greedy (T=0), `--max_tokens` (default 4096), rule-based grader (`eval/grade.py`, `<answer>` `\boxed` MCQ letter↔value from the question's options, **no LLM judge**). It writes one CSV row
...
|------|----|----|----|-----------------|-------------------|
| 8 (reference) | 1 | 8 | 64 | 8 | 8 yes |
| 4 | 2 | 8 | 64 | 8 | 8 yes |
| 2 | 4 | 8 | 64 | 8 | 8 yes |
| 2 (**wrong**) | 1 | 32 | 64 | 2 | forced to 2 (wrong) |
...
- **We-Math eval hangs / times out at ~4h** lower `--limit`/`--max_tokens` (this repo's `eval/eval_mllm.py` has no `--max_num_seqs` flag; for the proper fix add `max_num_seqs=64` to its `LLM(...)`). We-Math
  ...
  collapses (~2266 it/s). A bounded running batch is
