...
# smoke: MAX_STEPS=1 MAX_SAMPLES=64 bash examples/openr1_qwen25vl3b_gt.sh
set -euo pipefail
...
TS="$(date +%Y%m%d_%H%M%S)"; RUN="openr1_qwen25vl3b_gt_${TS}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}" accelerate launch \
    ...
    --num_processes ${NUM_PROC:-8} --main_process_port 19411 --gradient_accumulation_steps ${GA:-8} \
