#!/usr/bin/env bash
# open_r1 · gemma3_12b · google/gemma-3-12b-it · gt(stock GRPO) · stack B (torch2.9/vllm0.11.2/tf4.57.0/ds0.18), ZeRO-3+optim-offload.
...
# NOTE: Gemma uses --attn_implementation sdpa (NOT flash_attention_2). smoke: MAX_STEPS=1 MAX_SAMPLES=64 bash examples/openr1_gemma3_12b_gt.sh
set -euo pipefail
...
ar_find_run "mmr1_gemma3_12b_gt_mmupt"
RUN="mmr1_gemma3_12b_gt_mmupt"    # 一个实验一个固定目录,不带时间戳
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}" accelerate launch \
    ...
    --num_processes ${NPROC:-8} --main_process_port 19496 --gradient_accumulation_steps ${GA:-15} \
    ...
    --per_device_train_batch_size ${BS:-1} --gradient_accumulation_steps ${GA:-15} \
    ...
    --max_completion_length 1024 --num_generations 10 --temperature 1.0 \
    --use_vllm --vllm_mode colocate --vllm_max_model_length 4096 \
    --vllm_gpu_memory_utilization ${VLLM_MEM:-0.40} --vllm_importance_sampling_mode token_truncate \
