#!/usr/bin/env bash
# open_r1 · internvl35_8b · Qwen/Qwen2.5-VL-7B-Instruct · gt(stock GRPO) · stack B (torch2.9/vllm0.11.2/tf4.57.0/ds0.18), ZeRO-3+optim-offload.
...
ar_find_run "mmr1_qwen25vl7b_ttrl_b128"
RUN="mmr1_qwen25vl7b_ttrl_b128"    # 一个实验一个固定目录,不带时间戳
BASE_OUT="${MLLM_WORKDIR_ROOT:-work_dirs}/mllm-co-grpo-dp/$RUN"
mkdir -p "$BASE_OUT"
ar_resume_arg "$BASE_OUT"
MAXSTEPS_ARG="--max_steps ${MAX_STEPS:-90}"   # 90 步 = 2 epoch 评估点;续跑时调大
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}" accelerate launch \
    ...
    --num_processes ${NPROC:-8} --main_process_port 19497 --gradient_accumulation_steps ${GA:-80} \
    ...
    --per_device_train_batch_size ${BS:-2} --gradient_accumulation_steps ${GA:-80} \
    --num_train_epochs 15 ${MAXSTEPS_ARG} \
    ...
    --logging_steps 1 --save_strategy steps --save_steps ${SAVE_STEPS:-10} \
    --save_only_model ${SAVE_ONLY_MODEL:-false} \
    --save_total_limit ${SAVE_LIMIT:-3} \
    ...
    --attn_implementation flash_attention_2 --trust_remote_code --bf16 true \
