...
TS="$(date +%Y%m%d_%H%M%S)"; RUN="openr1_gemma3_12b_gt_${TS}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}" accelerate launch \
    ...
    --num_processes ${NUM_PROC:-8} --main_process_port 19461 --gradient_accumulation_steps ${GA:-4} \
    trainers/train_mllm_single.py \
    --model_name_or_path "$MODEL" --train_dataset "$DATASET" \
    --output_dir "$BASE_OUT" --run_config "$RUN" \
    --learning_rate 1e-6 \
    --per_device_train_batch_size ${BS:-2} --gradient_accumulation_steps ${GA:-4} \
    ...
    --attn_implementation sdpa --trust_remote_code --bf16 true 2>&1 | tee -a "$BASE_OUT/train.log"
