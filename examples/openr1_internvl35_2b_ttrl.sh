#!/usr/bin/env bash
# open_r1 / mmr1, internvl35_2b, OpenGVLab/InternVL3_5-2B-HF, ttrl (unmaj, self-label majority-vote), stack B (torch2.9/vllm0.11.2/tf4.57.0/ds0.18), ZeRO-3 (no offload).
...
# smoke: MAX_STEPS=1 MAX_SAMPLES=64 bash examples/openr1_internvl35_2b_ttrl.sh
set -euo pipefail
...
TS="$(date +%Y%m%d_%H%M%S)"; RUN="openr1_internvl35_2b_ttrl_${TS}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}" accelerate launch \
    ...
    --num_processes ${NUM_PROC:-8} --main_process_port 19414 --gradient_accumulation_steps ${GA:-8} \
