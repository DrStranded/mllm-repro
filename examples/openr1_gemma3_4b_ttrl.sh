#!/usr/bin/env bash
# open_r1 / mmr1, gemma3_4b, google/gemma-3-4b-it, ttrl (unmaj, self-label majority-vote), stack B (torch2.9/vllm0.11.2/tf4.57.0/ds0.18), ZeRO-3 (no offload).
...
# smoke: MAX_STEPS=1 MAX_SAMPLES=64 bash examples/openr1_gemma3_4b_ttrl.sh
set -euo pipefail
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}" accelerate launch \
    ...
    --num_processes ${NUM_PROC:-8} --main_process_port 19416 --gradient_accumulation_steps ${GA:-8} \
