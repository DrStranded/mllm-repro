...
ar_find_run "mmr1_qwen25vl7b_gt_mmupt"
RUN="mmr1_qwen25vl7b_gt_mmupt"    # 一个实验一个固定目录,不带时间戳
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}" accelerate launch \
    ...
    --num_processes ${NPROC:-8} --main_process_port 19497 --gradient_accumulation_steps ${GA:-5} \
