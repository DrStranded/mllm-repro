#!/usr/bin/env bash
# open_r1, qwen25vl7b, Qwen/Qwen2.5-VL-7B-Instruct, gt(stock GRPO), stack B (torch2.9/vllm0.11.2/tf4.57.0/ds0.18), ZeRO-3+optim-offload.
...
# smoke: bash examples/smoke.sh   (or: MAX_STEPS=1 MAX_SAMPLES=64 bash examples/openr1_qwen25vl7b_gt.sh)
set -euo pipefail
...
TS="$(date +%Y%m%d_%H%M%S)"; RUN="openr1_qwen25vl7b_gt_${TS}"
...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}" accelerate launch \
    ...
    --num_processes ${NUM_PROC:-8} --main_process_port 19469 --gradient_accumulation_steps ${GA:-4} \
