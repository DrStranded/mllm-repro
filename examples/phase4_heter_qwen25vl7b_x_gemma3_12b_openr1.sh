...
# smoke: MAX_STEPS=1 MAX_SAMPLES=64 bash examples/phase4_heter_qwen25vl7b_x_gemma3_12b_openr1.sh
set -euo pipefail
...
MODEL_A="Qwen/Qwen2.5-VL-7B-Instruct"; MODEL_B="google/gemma-3-12b-it"
VLLM_MEM_A="${VLLM_MEM:-0.45}"; VLLM_MEM_B="${VLLM_MEM:-0.45}"
TS="$(date +%Y%m%d_%H%M%S)"; RUN="phase4_heter_qwen25vl7b_x_gemma3_12b_openr1_${TS}"
...
launch_group A "0,1,2,3" "$MODEL_A" "$MODEL_B" 19472 "$BASE_OUT/model_a" "$VLLM_MEM_A" "flash_attention_2" & PID_A=$!
launch_group B "4,5,6,7" "$MODEL_B" "$MODEL_A" 19473 "$BASE_OUT/model_b" "$VLLM_MEM_B" "sdpa" & PID_B=$!
